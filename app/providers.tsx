                    ...
                    <View style={stylesModal.container}>
                        <View style={stylesModal.inputWrapper}>
                            ...
                            <TextInput
                                style={stylesModal.inputBox}
                                onChangeText={(text) =>{
                                    setProduct({ ...product, contact: text });
                                    setError("")
                                }
                                }
                        ...
                        <TouchableOpacity
                            onPress={() => {
                                if(product.name && product.contact){
                                    onSubmit(product);
                                    setProduct({});
                                    return;
                                }
...
export default function ProviderScreen(){
    ...
    useEffect(() => {
        const getProducts = async () => {
            const productsCollection = await collection(firestore, "provider");
    ...
    const getDocument = async (id:string) => {
        const document = await getDoc(doc(firestore, "provider", id));
        setProduct({...document.data(), id: document.id});
        setOpenModalProduct(true);
    }

    const deleteDocument = async (id:string) => {
        await deleteDoc(doc(firestore, "provider", id));
    ...
    return (
        <View style={styles.container}>
            ...
            <View>
                <SafeAreaView>
                    <FlatList
                        ListEmptyComponent={()=><Text>No hay proveedores</Text>}
                        data={providers}
                        style={styles.list}
                        renderItem={(data) => (
